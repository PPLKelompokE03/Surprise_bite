        ...
        <div class="mt-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            @forelse ($boxes as $box)
                @php
                    ...
                    $pct = $box['original_price'] > 0 ? (int) round(100 - ($box['price'] / $box['original_price']) * 100) : 0;
                @endphp
                <article class="overflow-hidden rounded-3xl bg-white shadow-md shadow-black/5 ring-1 ring-slate-100 transition hover:shadow-lg">
                    ...
                    <div class="p-5">
                        <p class="text-xs font-semibold text-[#6a7282]">{{ $box['category_label'] }} • {{ number_format($box['distance_km'], 1) }} km</p>
                        <h3 class="mt-1 text-lg font-black text-[#1e2939]">{{ $box['title'] }}</h3>
                        <p class="mt-0.5 text-sm text-[#6a7282]">{{ $r['name'] ?? '' }}</p>
